Ext.define('Ext.plugin.SignaturePad', {
	...
	init : function(cmp) {
		...
		Ext.Viewport.on("orientationchange",this.reInitCanvas,this);
	},
	reInitCanvas : function(viewport,newOrientation,width,height,opts){
		this.canvas.image = this.getSignatureAsImage('DATA');
		if(/[0-9]+/.test(newOrientation)){
			if(newOrientation > width){
				this.config.width = newOrientation - 10;
				this.config.height = height - 10;
			}else{
				this.config.width = newOrientation - 10;
				this.config.height = height - 10;
			}
		}else{
			this.config.width = width - 10;
			this.config.height = height - 10;
		}
		Ext.get(this.config.canvasId).dom.width = this.config.width;
		Ext.get(this.config.canvasId).dom.height = this.config.height;
		this.reset();
		var image = new Image();
		image.src = this.canvas.image;
		var that = this;
		image.onload = function() {
			//that.canvas.getContext("2d").drawImage(image,0,0);
		};
	},
	initCanvas : function(comp, opts) {
		if(!this.initializeOnce){
			...
			return false;
		this.config.width = this.config.width != undefined ? this.config.width : this.cmp.element.dom.offsetWidth ? this.cmp.element.dom.offsetWidth : this.cmp.element.dom.clientWidth;
		console.log(this.config.width);
		this.config.width -= 16;
		this.config.height = this.config.height != undefined ? this.config.height : this.cmp.element.dom.offsetHeight ? this.cmp.element.dom.offsetHeight : this.cmp.element.dom.clientHeight;
		console.log(this.config.height);
		this.config.height -= 16;
		this.cmp.setHtml('<canvas style="margin-top:8px;margin-left:8px" width="'
				+ this.config.width
				+ '" height="'
				+ this.config.height
				+ '" id="'
	...
	reset : function() {		
		...
		canvasBackground.fillStyle = '#fff';
		canvasBackground.fillRect(0,0,canvas.width, canvas.height);
		var that = this,
			img = new Image();
		/*img.addEventListener('load',onImgLoad,false);
		img.src = 'sdk/resources/images/watermark.png';
		function onImgLoad(){
			var imgWidth = img.width,
			imgHeight = img.height;
			ctx.drawImage(img,(that.config.width-imgWidth)/2,(that.config.height-imgHeight)/2);
			//ctx.drawImage(img,0,0);
			img.removeEventListener('load',onImgLoad,false);
		}*/
